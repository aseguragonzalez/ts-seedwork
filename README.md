...
| **Application**    | `Command`/`CommandBus`/`CommandHandler`, `Query`/`QueryBus`/`QueryHandler`, `Result`, `Maybe`, `DomainEventBusPublisher`/`DomainEventBusSubscriber`/`DomainEventBus`, `DomainEventBusContext`, `DomainEventHandler`, `IntegrationEvent`/`IntegrationEventHandler`/`IntegrationEventPublisher`, `BackgroundTask`/`TaskScheduler` |
