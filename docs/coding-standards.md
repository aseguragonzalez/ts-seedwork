...
- Implement `protected validate(): void`; throw a `DomainError` subclass there for invalid state; call `this.validate()` at end of constructor
...
- `Command` and `Query` are abstract classes; concrete subclasses extend them (`extends Command`, not `implements Command`)
- Implement `protected validate(): void`; throw `ValidationErrors` inside it
- Call `this.validate()` at the end of the constructor — validation fires at construction time
...
Three decorators compose the stack. Declaration order in `CommandBusBuilder` determines wrapping: **first declared = outermost** (runs first on every `dispatch` call).

Dispatch execution order:

```
Transactional → DomainEventCoordinator → Registry
```
...
const commandBus = new CommandBusBuilder()
  ...
  .withTransaction(unitOfWork) // outermost — opens transaction first
...
1. `TransactionalCommandBus` opens a UoW transaction
2. `DomainEventCoordinatorCommandBus` calls `dispatch()` after the handler returns, `discard()` on rollback
3. `RegistryCommandBus` routes to the correct `CommandHandler.handle()`

Validation happens before the bus is involved — `new OpenAccountCommand(...)` throws `ValidationErrors` at construction time if the input is invalid.
